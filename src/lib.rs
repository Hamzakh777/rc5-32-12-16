...
pub fn expand_key(key: &Vec<u8>) -> Result<Vec<u32>, RC5Error> {
    ...
    for i in (0..B).rev() {
        ...
        words[current_word_index] = (words[current_word_index].wrapping_shl(8u32)).wrapping_add(key[i] as u32);
    }
    ...
    for _ in 0..iterations  {
		a = subkeys[subkeys_index].wrapping_add(a.wrapping_add(b)).rotate_left(3);
        subkeys[subkeys_index] = a;
        b = words[words_index].wrapping_add(a.wrapping_add(b)).rotate_left(a.wrapping_add(b) % W as u32);
        words[words_index] = b;
		// reset the indexes 
...
fn encode(key: Vec<u8>, plaintext: Vec<u8>) -> Result<Vec<u8>, RC5Error> {
    ...
    for i in 1..=R {
		a = a ^ b;
        a = a.rotate_left(b % W as u32).wrapping_add(expanded_key[2 * i]);
        b = b ^ a;
        b = b.rotate_left(a % W as u32).wrapping_add(expanded_key[2 * i + 1]);
...
fn decode(key: Vec<u8>, ciphertext: Vec<u8>) -> Result<Vec<u8>, RC5Error> {
    ...
    for i in (1..=R).rev() {
        b = b.wrapping_sub(expanded_key[2 * i + 1]).rotate_right(a % W as u32) ^ a;
        a = a.wrapping_sub(expanded_key[2 * i]).rotate_right(b % W as u32) ^ b
    }

    a.wrapping_sub(expanded_key[0]).to_le_bytes().map(|el| plaintext.push(el));
	b.wrapping_sub(expanded_key[1]).to_le_bytes().map(|el| plaintext.push(el));
